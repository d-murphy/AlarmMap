...
shinyServer(function(input, output) {
  ...
  map <- reactive({
      ...
      Map <- qmap(input$`Map Center`,
                  zoom=13, color="bw", 
                  legend="bottomright" )  #,size = c(1200,1200)
...
#      if(is.null(brushdf)){
    ...
    ResData <- reactive({
          ...
          ResData <- resdf %>% filter(RunNumber %in% data()$RunNumber)
...
#    DecCt <- reactive({ data() %>% filter(month(Date) == 12) %>% summarize(ct = n()) })    
    ...
    output$distPlot <- renderPlot({
       ...
       map() + geom_point(aes(x = lon, y=lat, fill=SignalTypeAbbr, colour=SignalTypeAbbr),
                        data = data(), alpha=.8, size = 2) + 
